...
class DataTrainingArguments:
    ...
    source_column: Optional[str] = field(
        default="input",  # 'input' sütunu veri setinizdeki hatalı cümleleri içerir
        metadata={"help": "The name of the column in the dataset containing the source texts."},
    )
    target_column: Optional[str] = field(
        default="target",  # 'target' sütunu veri setinizdeki düzeltilmiş cümleleri içerir
    ...
    def __post_init__(self):
        if self.dataset_name is None and self.train_file is None and self.validation_file is None:
            ...
            if self.train_file is not None:
                ...
                assert extension in ["csv", "json", "tsv"], "`train_file` should be a csv, tsv or a json file."
            if self.validation_file is not None:
                extension = self.validation_file.split(".")[-1]
                assert extension in ["csv", "json", "tsv"], "`validation_file` should be a csv, tsv or a json file."
...
def main():
    ...
    if data_args.dataset_name is not None:
        # Hugging Face Hub'dan veri setini yükle
        raw_datasets = load_dataset(
            data_args.dataset_name, data_args.dataset_config_name, cache_dir=model_args.cache_dir, use_auth_token=model_args.use_auth_token
        )
    else:
    ...
    source_column = data_args.source_column if data_args.source_column is not None else column_names[0]
    target_column = data_args.target_column if data_args.target_column is not None else column_names[1]

    if source_column not in column_names or target_column not in column_names:
        raise ValueError(
            f"Specified source_column '{source_column}' or target_column '{target_column}' "
    ...
    def compute_metrics(eval_preds):
        ...
        logger.info(f"Eval batch avg. Levenshtein distance: {lev_dist_avg}")
        prediction_lens = [np.count_nonzero(pred != tokenizer.pad_token_id) for pred in preds]
        result["gen_len"] = np.mean(prediction_lens)
        results["Levenshtein"] = lev_dist_avg
